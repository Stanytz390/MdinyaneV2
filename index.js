...
// -------------------- Default data files --------------------
const phoneNumber = config.pairingNumber || config.ownerNumber || "255620490076";
...
global.botname = config.botName || "ᴍᴅɪɴʏᴀɴᴇ ᴠ2";
global.themeemoji = "•";
...
function hasValidSession() {
  try {
    ...
    if (!existsSync(credsPath)) {
      printLog('warning', 'creds.json does not exist');
      return false;
    }
    const fileContent = fs.readFileSync(credsPath, 'utf8');
    if (!fileContent || fileContent.trim().length === 0) {
      printLog('warning', 'creds.json is empty');
      return false;
    }
    const creds = JSON.parse(fileContent);
    // Required fields according to Baileys
    if (!creds.noiseKey || !creds.signedIdentityKey || !creds.signedPreKey) {
      printLog('warning', 'creds.json missing required fields');
      return false;
    }
...
/**
 * Attempt to download and save credentials using the session ID.
 * Returns true if successful, false otherwise.
 */
async function downloadSessionCredentials(sessionId) {
  try {
    printLog('info', `Downloading session from ID: ${sessionId}`);
...
async function initializeSession() {
  ...
  printLog('info', 'No valid session found. Attempting to download from Pastebin/paste.rs...');
  const downloaded = await downloadSessionCredentials(sessionId);
  if (downloaded) {
    return true;
  } else {
  ...
  try {
    ...
    QasimDev.getName = (jid, withoutContact = false) => {
      ...
      if (id.endsWith("@g.us")) {
        ...
        v = id === '0@s.whatsapp.net' ? { id, name: 'WhatsApp' }
          : id === QasimDev.decodeJid(QasimDev.user.id) ? QasimDev.user
          : (store.contacts[id] || {});
        return (withoutContact ? '' : v.name) || v.subject || v.verifiedName || PhoneNumber(`+${id.replace('@s.whatsapp.net', '')}`).number?.international;
      }
    };
    ...
    QasimDev.ev.on('connection.update', async (s) => {
      ...
      if (connection === "open") {
        ...
        printLog('info', `[ ${config.botName || 'ᴍᴅɪɴʏᴀɴᴇ ᴠ2'} ]`);
